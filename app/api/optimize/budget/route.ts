...
import { z } from "zod";
import { optimizeBudget } from "@/lib/ai/features/service";
...
const Schema = z.object({
  ...
  targetReductionPercent: z.number().min(5).max(60),
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await optimizeBudget(parsed.data as unknown as BudgetOptimizationRequest);
